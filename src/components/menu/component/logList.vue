<template>
  <div style="padding:25px 25px 0 25px;" class="text">
     <div><img :src="baseURL+postItem.logo" alt=""></div>
     <div style="padding:10px 0" v-show="postItem.capital_ending_price">
         <span style="color:#666;font-size:16px;">{{news.SD}}：{{postItem.currency }} </span>
         <span style="color:#2F9AC0;font-size:16px;">{{postItem.capital_ending_price}}-{{postItem.capital_starting_price}}</span>
         <span class="hiddenT" style="color:#999;font-size:16px;">( {{news.SZ}} )</span>
     </div>
     <div style="padding:10px 0" v-show="postItem.other_starting_price">
         <span style="color:#666;font-size:16px;">{{news.QT}}：{{postItem.currency }}</span>
         <span style="color:#2F9AC0;font-size:16px;"> {{postItem.other_starting_price}}-{{postItem.other_ending_price}}</span>
         <span class="hiddenT" style="color:#999;font-size:16px;">( {{news.SZ}} )</span>
     </div>
     <div class="flexs j-between a-center" style="padding:10px 0">
